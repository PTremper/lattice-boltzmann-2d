...
This module is written in NumPy.
"""
...
class LatticeBoltzmann2D:
    ...
    def __init__(
        ...
        # Random number generator for initialization noise
        ...
        self.c: NDArray[np.integer] = np.array([(d.cx, d.cy) for d in DIRECTIONS])

        # Corresponding lattice weights w_i (shape: (9,))
        self.weights: NDArray[np.floating] = np.array([d.w for d in DIRECTIONS])

        # Opposite direction indices (used for bounce-back boundary conditions)
        # Example: east (1) <-> west (3), north (2) <-> south (4), etc.
        self.opp: NDArray[np.integer] = np.array([0, 3, 4, 1, 2, 7, 8, 5, 6])

        # -------------------------------------------------------------------
        # Geometry and domain setup
        # -------------------------------------------------------------------
        self.solid: NDArray[np.bool_] = geometry.astype(bool)
        ...
        self.rho: NDArray[np.floating] = rho0 * np.ones((self.nx, self.ny))
        self.u: NDArray[np.floating] = np.zeros((self.nx, self.ny, 2))

        # Distribution function f (shape: (nx, ny, 9))
        self.f: NDArray[np.floating] = np.zeros((self.nx, self.ny, 9))
    ...
    def _macroscopic(self) -> None:
        ...
        j = np.sum(self.f[..., :, None] * self.c, axis=2)

        # Velocity (shape: (nx, ny, 2))
        self.u = j / self.rho[..., None]
    ...
    def _equilibrium(self) -> NDArray[np.floating]:
        ...
        cu = np.einsum("ia,xya->xyi", self.c, self.u)
...
if __name__ == "__main__":
    ...
    lbm = LatticeBoltzmann2D(geometry)
